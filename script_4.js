...
console.log("Combien d'handles contiennent un numéro ?");
  function journalist_with_number(journalists) {
    let regex = /\d+/;
    let journalist_with_number = [];
      journalists.forEach(journalist => {
        if (journalist.match(regex)) {
          journalist_with_number.push(journalist);
        }
      });
    console.log(journalist_with_number.length);
  };
journalist_with_number(journalists)

console.log('Combien dhandles contiennent les 4 lettres du prénom Aude à la suite (sans prendre en compte les majuscules ?');
  function journalist_with_aude(journalists) {
  let journalist_with_aude = [];
    journalists.forEach(journalist => {
      if (journalist.toUpperCase().match("Aude".toUpperCase())) {
        journalist_with_aude.push(journalist);
      }
    });
      console.log(journalist_with_aude.length);
  };
journalist_with_aude(journalists)
...
console.log('Combien contiennent une majuscule ?');
  function journalists_with_total_maj(journalists) {
    let regex = /[A-Z]/;
    let journalist_with_maj = [];
      journalists.forEach(journalist => {
        if (journalist.match(regex)) {
