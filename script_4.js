...
console.log(`Il y a ${journalists.length} journalistes dans cet array.`);

/* 2 */
console.log("Combien d'handles contiennent un numéro ?");
let handleWithNumbers = journalists.filter(value => /[0-9]/.test(value)); // Noms qui contiennent des numéros
console.log(`Il y a visiblement ${handleWithNumbers.length} noms de journalistes qui contiennent un nombre.`);

/* 3 */
console.log('Combien dhandles contiennent les 4 lettres du prénom Aude à la suite (sans prendre en compte les majuscules ?');
let handleWithAude = journalists.filter(value => /aude/i.test(value)); // Noms qui contiennent 'Aude'
console.log(`Il y a ${handleWithAude.length} noms qui contiennent 'Aude'.`);
...
let handleWithUpper = journalists.filter(value => /[A-Z]/.test(value)); // Noms qui contiennent une majuscule
console.log(`Il y a ${handleWithUpper.length} noms qui contiennent une majuscule.`);
